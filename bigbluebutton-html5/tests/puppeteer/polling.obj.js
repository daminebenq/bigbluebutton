...
=======
  ...
  // This Test chooses A/B/C/D polling case, runs it
  // and drags a file and drops it in dragAndDropPollContainer 
  // which contains possible answers, then it fills poll 
  // answers options from the dragged file, starts the poll
  // and finally publishes the poll results
  test('Drag & Drop File with poll options', async () => {
    const test = new Multiusers();
    let response;
    let screenshot;
    try {
      const testName = 'dragAndDrop';
      await test.page1.logger('begin of ', testName);
      await test.init(undefined, testName);
      response = await test.dragAndDrop(testName);
      await test.page1.logger('end of ', testName);
>>>>>>> Stashed changes
      screenshot = await test.page1.page.screenshot();
    } catch (e) {
