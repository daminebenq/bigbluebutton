...
class MultiUsers {
  ...
  async dragAndDrop(testName) {
    ...
    await this.page1.click(ple.a4Btn, true);

    const fileToUpload = path.join(__dirname, '../media/poll-options.txt');
    // await this.page1.page.waitForSelector(fileToUpload);
    await this.page1.page.waitForSelector(ple.dragAndDropArea);
    const origin = await this.page1.page.$(fileToUpload);
    const destination = await this.page1.page.$(ple.dragAndDropArea);
    ...
    await this.page1.page.mouse.move(ob.x + ob.width / 2, ob.y + ob.height / 2);
    await this.page1.page.mouse.down();
    console.log(`Dropping at   ${db.x + db.width / 2}, ${db.y + db.height / 2}`);
    await this.page1.page.mouse.move(db.x + db.width / 2, db.y + db.height / 2);
    await this.page1.page.mouse.up();

    await this.page1.click(ple.fillPollValues, true);

    console.log('GOT HERE !');

    await this.page2.clickNItem(ple.pollAnswerOptionBtn, true, 2);

    await this.page1.stopRecording();
